...
Don't bother showing projectile in the minibuffer, I have enough in
there already.
...
#+BEGIN_SRC emacs-lisp :tangle yes :comments org
  (defvar wh/original-frame-title-format "%b - emacs")

  (defun wh/flycheck-or-frame-title ()
    ...
    (let* ((flycheck-errs (flycheck-overlay-errors-at (point)))
      ...
      (if first-err
          (let* ((pretty-err (flycheck-error-format-message-and-id first-err))
                 (first-line (-first-item (s-lines pretty-err))))
            (format "FlyC: %s" first-line))
        (format-mode-line wh/original-frame-title-format))))

  (setq frame-title-format `(:eval (wh/flycheck-or-frame-title)))
#+END_SRC
